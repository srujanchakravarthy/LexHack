 ...
 */
 ...
 function validateSSN(SSN, EID) {
    ...
    if (SSN) {if(SSN!==null && (SSNInternal<1 || SSNInternal>3)){
        return buildValidationResult(false, 'slotThree', 'Please enter severity between 1-3');
    }
    else if(isNaN(SSNInternal))
    {
        return buildValidationResult(false, 'slotThree', 'Please enter only numbers');
    }
   }
    if (EID && EID!==null) {if(EID.length != 7 && !isNaN(EIDInternal)){
        return buildValidationResult(false, 'EmpID', 'Please enter only 7 digits');
    }
    else if(isNaN(EIDInternal))
    {
        return buildValidationResult(false, 'EmpID', 'Please enter only numbers');
    }
    }
...
function makeSSN(intentRequest, callback) {
    const SSN = intentRequest.currentIntent.slots.slotThree;
    const EID = intentRequest.currentIntent.slots.EmpID;
    console.log(SSN);
    console.log(EID);
...
console.log("Adding a new item...");*/
    ...
    var eParams = {
            ...
            Message: {
                Body: {
                    Text: {
                        Data: "Your trouble ticket has been put in our queue. We will work on a resolution soon.  Thanks, Your Bot"
                    }
                },
                Subject: {
...
*/
       ...
       'content': `We have raised a ticket ${hexstring} for Admin on this. Thanks for reaching us` }));
    }
